...
function IndexCardComponent({ card }: { card: IndexCard }) {
    ...
    return (
        <div className="flex-1 min-w-0 bg-gradient-to-br from-[#1a2a22] to-[#111c18] border border-[#2a3d30]/50 rounded-3xl p-4 flex flex-col gap-1.5 shadow-lg">
            <span className="text-sm font-bold text-[#a8a8a0] tracking-wider uppercase">{card.label}</span>
            <span className="text-xl font-bold leading-tight">
                {card.price > 0 ? card.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "—"}
            </span>
            <span className={`text-sm font-semibold ${pos ? "text-[#4ade9a]" : "text-red-500"}`}>
                {pos ? "▲" : "▼"} {Math.abs(card.pct).toFixed(2)}%
            </span>
            <svg viewBox="0 0 100 50" className="w-full h-10 mt-1" preserveAspectRatio="none">
                <defs>
...
function MoverRow({ item, router, showVol }: { item: MoverItem; router: ReturnType<typeof useRouter>; showVol?: boolean }) {
    ...
    return (
        <div
            ...
            className="flex items-center px-5 py-4 hover:bg-[#1a2a22] cursor-pointer transition-colors border-b border-[#2a3d30]/50 last:border-0"
        >
            <div className="flex flex-col flex-1 min-w-0">
                <span className="font-bold text-base text-[#f0ede8]">{item.symbol}</span>
                <span className="text-sm text-[#a8a8a0] truncate">{item.name}</span>
                {showVol && <span className="text-sm text-[#a8a8a0] mt-0.5">Vol: {fmtVol(item.volume)}</span>}
            </div>
            <div className="flex flex-col items-end">
                <span className="font-semibold text-base text-[#f0ede8]">${item.price.toFixed(2)}</span>
                <span className={`text-sm font-bold ${pos ? "text-[#4ade9a]" : "text-red-500"}`}>
            ...
            <div className="mt-4">
                <div className="bg-[#111c18] border border-[#2a3d30]/50 rounded-3xl p-6 shadow-lg">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-sm text-[#a8a8a0]">Advance / Decline</span>
                    </div>
                    <div className="flex items-center gap-3 mb-2">
                        <span className="font-bold text-[#4ade9a] text-base">↑ {breadth.up.toLocaleString()}</span>
                        <span className="text-[#a8a8a0] text-sm">vs</span>
                        <span className="font-bold text-red-500 text-base">↓ {breadth.down.toLocaleString()}</span>
                    </div>
                    <div className="w-full h-3 rounded-full overflow-hidden flex bg-red-500/20">
                        <div
                    ...
                    <div className="flex justify-between mt-1.5">
                        <span className="text-xs text-[#a8a8a0]">{((breadth.up / breadthTotal) * 100).toFixed(0)}% advancing</span>
                        <span className="text-xs text-[#a8a8a0]">{((breadth.down / breadthTotal) * 100).toFixed(0)}% declining</span>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-4">
                <div className="bg-[#111c18] border border-[#2a3d30]/50 rounded-3xl p-6 shadow-lg">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <span className="text-sm font-bold" style={{ color: fearGreedColor }}>{fearGreedLabel}</span>
                    </div>
                    <div className="relative w-full h-4 rounded-full bg-gradient-to-r from-red-600 via-amber-400 to-[#4ade9a]">
                        <div
                    ...
                    <div className="flex justify-between mt-1.5">
                        <span className="text-xs text-red-500 font-semibold">Extreme Fear</span>
                        <span className="text-2xl font-bold" style={{ color: fearGreedColor }}>{fearGreed}</span>
                        <span className="text-xs text-[#4ade9a] font-semibold">Extreme Greed</span>
                    </div>
                </div>
            </div>
            ...
            <div ref={searchRef} className="relative mt-5">
                <div className="relative flex items-center bg-[#1a2a22] border border-[#2a3d30] rounded-xl py-3 pl-4 pr-3 gap-3 transition-all focus-within:border-[#4ade9a]/50 focus-within:ring-2 focus-within:ring-[#4ade9a]/20">
                    ...
                    <input
                        ...
                        className="flex-1 min-w-0 bg-transparent text-[#f0ede8] placeholder-[#a8a8a0] text-base focus:outline-none"
                    />
                    {searchResults.length > 0 && (
                        <span className="shrink-0 text-[#a8a8a0] text-base font-medium bg-[#0a120f] border border-[#2a3d30]/50 rounded-full px-3 py-1">
                    ...
                    <div className="absolute top-full left-4 right-4 mt-1 bg-[#111c18] border border-[#2a3d30] rounded-xl overflow-hidden shadow-2xl z-50 max-h-64 overflow-y-auto">
                        {searchResults.length > 0 ? searchResults.map(r => (
                            <button key={r.symbol} type="button"
                                ...
                                className="w-full flex justify-between items-center px-4 py-3.5 hover:bg-[#1a2a22] text-left transition">
                                <div>
                                    <span className="font-bold text-base text-[#f0ede8]">{r.symbol}</span>
                                    <span className="text-[#a8a8a0] text-base ml-2">{r.name}</span>
                                </div>
                                <span className="text-[#a8a8a0] text-base">View →</span>
                            </button>
                        )) : <div className="px-4 py-6 text-center text-[#a8a8a0] text-base">No results found</div>}
                    </div>
                )}
            </div>
            ...
            <div className="mt-5">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                    {watchlistData.length === 0
                        ? WATCHLIST.map(s => (
                            <div key={s} className="flex flex-col items-center bg-[#111c18] border border-[#2a3d30]/50 rounded-2xl px-5 py-3 min-w-[80px] shrink-0 animate-pulse">
                                <span className="font-bold text-base text-[#f0ede8]">{s}</span>
                                <span className="text-sm text-[#a8a8a0] mt-1">—</span>
                            </div>
                        ))
                        : watchlistData.map(w => (
            ...
            <div className="mt-5">
                ...
                <div className="bg-[#111c18] border border-[#2a3d30]/50 rounded-3xl overflow-hidden shadow-lg">
                    {moversLoading ? (
                        ...
                        <div className="px-4 py-8 text-center text-[#a8a8a0] text-base">No data available</div>
                    ) : (
